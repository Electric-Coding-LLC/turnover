...
- Make `Summary` the default home because it combines import status, totals, and
  personal records.
...
- tap refresh to stay on `Summary` while data updates
...
- From `Summary`, refresh and unit-toggle actions should not navigate away from
  the screen.
...
- A navigation flow that can guide implementation
