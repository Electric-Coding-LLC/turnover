...
- Keep naming and tagging attached to a run rather than split into separate
  management areas.
- Treat permission, loading, empty, and error handling as UI states layered onto
  core screens, not as a large set of extra screens.
...
- A screen list with a clear purpose for each screen
